...
class Student(Human):
    ...
    def who_am_i():
        print("я ученик")
    def hello(self):
        print("привет, мне "+str(self.age)+" лет, меня зовут "+self.name+" "+self.surname+" "+self.second_name+",мой средний балл "+mark)
class Teacher(Human):
    def who_am_i():
        print("я учитель")
    ...
    def hello(self):
        print("привет, мне "+str(self.age)+" лет, меня зовут "+self.name+" "+self.surname+" "+self.second_name+",я веду урок "+subject)
students=dict()
teachers=dict()
print("ученики")
for i in range(1,6):
